...
def check_manifests(ci_type):
    ...
    manifest_path = release_dir / "umkoind.manifest"
    cmd_umkoind_manifest = [
        "mt.exe",
        "-nologo",
        f"-inputresource:{release_dir / 'umkoind.exe'}",
        f"-out:{manifest_path}",
    ]
    run(cmd_umkoind_manifest)
...
def run_tests(ci_type):
    ...
    if ci_type == "standard":
        test_envs = {
            "UMKOIN_BIN": "umkoin.exe",
            "UMKOIND": "umkoind.exe",
            "UMKOINCLI": "umkoin-cli.exe",
            "UMKOIN_BENCH": "bench_umkoin.exe",
            "UMKOINTX": "umkoin-tx.exe",
            "UMKOINUTIL": "umkoin-util.exe",
            "UMKOINWALLET": "umkoin-wallet.exe",
            "UMKOINCHAINSTATE": "umkoin-chainstate.exe",
        }
        ...
        os.environ["UMKOINFUZZ"] = os.path.join(release_bin, "fuzz.exe")
        fuzz_cmd = [
            sys.executable,
