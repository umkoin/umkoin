...
# Be aware that umkoind and umkoin-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in umkoind.
#
...
# C locale.
#
# In contrast, umkoind does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where umkoind is guaranteed to be running
# with the classic locale ("C") whereas the locale of umkoin-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in umkoind but
# "1,230000" in umkoin-qt.
#
...
KNOWN_VIOLATIONS=(
    "src/umkoin-tx.cpp.*stoul"
    "src/umkoin-tx.cpp.*trim_right"
