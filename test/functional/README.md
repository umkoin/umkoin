...
- Use `platform.system()` for detecting the running operating system and `os.name` to
  ...
  methods in the `UmkoinTestFramework` class, which can be used to skip a whole test
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x50=1250 UMK) in its wallet. Using them is much more
...
with the umkoind(s) being tested in a callback-based event loop; the other
...
The [`TestShell`](test-shell.md) class exposes the UmkoinTestFramework
...
Test objects for interacting with a umkoind node over the p2p interface.
