...
# The block reward of coinbaseoutput.nValue (50) UMK/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 UMK/block.
...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"umkoin-cli -h\" for help or \"umkoin-cli listwallets\" to see "
...
class TestUmkoinCli(UmkoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `umkoin-cli getblockchaininfo`")
        ...
        # Prevent umkoin-cli from using existing rpcport in conf
        ...
        assert_raises_process_error(1, "Cookie file credentials were invalid and no rpcpassword was specified.", self.nodes[0].cli(f"-rpccookiefile={self.nodes[0].datadir_path / 'umkoin.conf'}").echo)
        ...
        self.log.info("Test -getinfo and umkoin-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and umkoin-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (UMK/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 UMK) to the default wallet.
            ...
            # Test umkoin-cli -generate with -rpcwallet in multiwallet mode.
