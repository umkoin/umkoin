...
class FeatureFrameworkStartupFailures(UmkoinTestFramework):
    ...
    def run_test(self):
        ...
        self._verify_startup_failure(
            ...
            r"AssertionError: \[node 0\] Unable to connect to umkoind after \d+s \(ignored errors: {[^}]*'OSError \w+'?: \d+[^}]*}, latest error: \w+\([^)]+\)\)"
        )
...
class TestWrongRpcPortStartupFailure(InternalTestMixin, UmkoinTestFramework):
    def add_options(self, parser):
