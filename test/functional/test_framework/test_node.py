...
# 'umkoin' command, it may internally insert more args, which must be
...
class TestNode():
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            ...
            try:
                ...
                if error_num is None:
                    ...
                    # Windows can raise this while umkoind shuts down during startup.
                ...
                # Suppress if cookie file isn't generated yet and no rpcuser or rpcpassword; umkoind may be starting.
                if "No RPC credentials" not in str(e):
                    raise
                latest_error = suppress_error("missing_credentials", e)
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error(f"Unable to connect to umkoind after {self.rpc_timeout}s (ignored errors: {dict(suppressed_errors)!s}{'' if latest_error is None else f', latest: {latest_error[0]!r}/{latest_error[1]}'})")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        Will raise if umkoind starts without an error.
        Will raise if an expected_msg is provided and it does not match umkoind's stdout."""
