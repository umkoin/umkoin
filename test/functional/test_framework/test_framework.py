...
class UmkoinTestFramework(metaclass=UmkoinTestMetaClass):
    ...
    def add_nodes(self, num_nodes, extra_args=None, rpchost=None, timewait=None, binary=None):
        ...
        for i in range(num_nodes):
            self.nodes.append(TestNode(i, get_datadir_path(self.options.tmpdir, i), rpchost=rpchost, timewait=timewait, umkoind=binary[i], umkoin_cli=self.options.umkoincli, mocktime=self.mocktime, coverage_dir=self.options.coveragedir, extra_conf=extra_confs[i], extra_args=extra_args[i], use_cli=self.options.usecli))
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            for i in range(MAX_NODES):
                ...
                self.nodes.append(TestNode(i, get_datadir_path(self.options.cachedir, i), extra_conf=["bind=127.0.0.1"], extra_args=[], rpchost=None, timewait=None, umkoind=self.options.umkoind, umkoin_cli=self.options.umkoincli, mocktime=self.mocktime, coverage_dir=None))
