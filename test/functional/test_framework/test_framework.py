...
class UmkoinTestFramework(metaclass=UmkoinTestMetaClass):
    ...
    def add_nodes(self, num_nodes, extra_args=None, *, rpchost=None, binary=None):
        ...
        if binary is None:
            binary = [self.options.umkoind] * num_nodes
        assert_equal(len(extra_confs), num_nodes)
        assert_equal(len(extra_args), num_nodes)
        assert_equal(len(binary), num_nodes)
        for i in range(num_nodes):
            self.nodes.append(TestNode(i, get_datadir_path(self.options.tmpdir, i), rpchost=rpchost, timewait=self.rpc_timewait, umkoind=binary[i], umkoin_cli=self.options.umkoincli, mocktime=self.mocktime, coverage_dir=self.options.coveragedir, extra_conf=extra_confs[i], extra_args=extra_args[i], use_cli=self.options.usecli))
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            # Create cache directories, run umkoinds:
            for i in range(MAX_NODES):
                datadir = initialize_datadir(self.options.cachedir, i)
                args = [self.options.umkoind, "-datadir=" + datadir]
                if i > 0:
                    args.append("-connect=127.0.0.1:" + str(p2p_port(0)))
                self.nodes.append(TestNode(i, get_datadir_path(self.options.cachedir, i), extra_conf=["bind=127.0.0.1"], extra_args=[], rpchost=None, timewait=self.rpc_timewait, umkoind=self.options.umkoind, umkoin_cli=self.options.umkoincli, mocktime=self.mocktime, coverage_dir=None))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i)  # Overwrite port/rpcport in umkoin.conf
...
def skip_if_no_umkoind_zmq(test_instance):
