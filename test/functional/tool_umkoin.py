...
class ToolUmkoinTest(UmkoinTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 1

    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `umkoin node -version` is
        # run on windows, python doesn't capture output from the child
        # `umkoind` and `umkoin-node` process started with _wexecvp, and
    ...
    def run_test(self):
        ...
        self.log.info("Ensure umkoin node command invokes umkoind by default")
        self.test_args([], [], expect_exe="umkoind")

        self.log.info("Ensure umkoin -M invokes umkoind")
        self.test_args(["-M"], [], expect_exe="umkoind")

        self.log.info("Ensure umkoin -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure umkoin -m invokes umkoin-node")
            self.test_args(["-m"], [], expect_exe="umkoin-node")

            self.log.info("Ensure umkoin -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="umkoin-node")

            self.log.info("Ensure umkoin accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="umkoin-node")

            self.log.info("Ensure umkoin recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="umkoin-node")
