...
class ConfArgsTest(UmkoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        with open(os.path.join(self.nodes[0].datadir, 'umkoin.conf'), 'a', encoding='utf-8') as conf:
    ...
    def test_config_file_log(self):
        ...
        node = self.nodes[0]
        conf_text = pathlib.Path(node.umkoinconf).read_text()
    ...
    def test_ignored_conf(self):
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir}" contains a "umkoin.conf" file which is ignored, because a '
        ...
        # error.
        self.start_node(0, [f'-conf={node.datadir}/umkoin.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        node = self.nodes[0]
        (default_datadir / "umkoin.conf").write_text(f"datadir={node.datadir}\n")
    ...
    def run_test(self):
        ...
        conf_file = os.path.join(default_data_dir, "umkoin.conf")
        ...
        none_existent_conf_file = os.path.join(default_data_dir, "none_existent_umkoin.conf")
