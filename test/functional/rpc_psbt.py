...
class PSBTTest(UmkoinTestFramework):
    ...
    def run_test(self):
        # Create and fund a raw tx for sending 10 UMK
        ...
        self.log.info("Test walletcreatefundedpsbt feeRate of 0.1 UMK/kB produces a total fee at or slightly below -maxtxfee (~0.05290000)")
        ...
        for unit, fee_rate in {"umk/kb": 0.1, "sat/b": 10000}.items():
        ...
        for mode in ["unset", "economical", "conservative", "umk/kb", "sat/b"]:
            ...
            if mode in ["umk/kb", "sat/b"]:
                assert_raises_rpc_error(-3, "Amount out of range",
                    lambda: self.nodes[1].walletcreatefundedpsbt(inputs, outputs, 0, {"estimate_mode": mode, "conf_target": -1, "add_inputs": True}))
                assert_raises_rpc_error(-4, "Fee rate (0.00000000 UMK/kB) is lower than the minimum fee rate setting (0.00001000 UMK/kB)",
        ...
        for unit, fee_rate in {"SAT/B": 0.99999999, "UMK/KB": 0.00000999}.items():
            ...
            assert_raises_rpc_error(-4, "Fee rate (0.00000999 UMK/kB) is lower than the minimum fee rate setting (0.00001000 UMK/kB)",
                lambda: self.nodes[1].walletcreatefundedpsbt(inputs, outputs, 0, {"estimate_mode": unit, "conf_target": fee_rate, "add_inputs": True}))

        self.log.info("Test walletcreatefundedpsbt feeRate of 10 UMK/kB produces total fee well above -maxtxfee and raises RPC error")
