...
UMKOIN_CONFIG_ALL="-DCMAKE_COMPILE_WARNING_AS_ERROR=ON -DBUILD_BENCH=ON -DBUILD_FUZZ_BINARY=ON"
if [ -z "$NO_DEPENDS" ]; then
...
eval "CMAKE_ARGS=($UMKOIN_CONFIG_ALL $UMKOIN_CONFIG)"
...
if [[ "$CI_OS_NAME" == "macos" && "${GOAL}" = "install deploy" ]]; then
  unzip "${BASE_BUILD_DIR}/umkoin-macos-app.zip" -d "${BASE_BUILD_DIR}/deploy"
  if ! ( codesign --verify "${BASE_BUILD_DIR}/deploy/Umkoin-Qt.app" ); then
...
if [ "${RUN_TIDY}" = "true" ]; then
  ...
  if ! ( run-clang-tidy-"${TIDY_LLVM_V}" -config-file="${BASE_ROOT_DIR}/src/.clang-tidy" -quiet -load="/tidy-build/libumkoin-tidy.so" "${MAKEJOBS}" | tee tmp.tidy-out.txt ); then
