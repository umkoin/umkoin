...
UMKOIN_CONFIG_ALL="--disable-dependency-tracking"
if [ -z "$NO_DEPENDS" ]; then
...
if [ -n "$ANDROID_TOOLS_URL" ]; then
  make distclean || true
  ./autogen.sh
  bash -c "./configure $UMKOIN_CONFIG_ALL $UMKOIN_CONFIG" || ( (cat config.log) && false)
...
bash -c "${BASE_ROOT_DIR}/configure --cache-file=config.cache $UMKOIN_CONFIG_ALL $UMKOIN_CONFIG" || ( (cat config.log) && false)

make distdir VERSION="$HOST"

cd "${BASE_BUILD_DIR}/umkoin-$HOST"
...
if [ "${RUN_TIDY}" = "true" ]; then
  ...
  set -eo pipefail
  cd "${BASE_BUILD_DIR}/umkoin-$HOST/src/"
  ...
  python3 "/include-what-you-use/iwyu_tool.py" \
           -p . "${MAKEJOBS}" \
           -- -Xiwyu --cxx17ns -Xiwyu --mapping_file="${BASE_BUILD_DIR}/umkoin-$HOST/contrib/devtools/iwyu/umkoin.core.imp" \
