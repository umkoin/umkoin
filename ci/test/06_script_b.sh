...
if [ "$RUN_FUZZ_TESTS" = "true" ]; then
  ...
  if [ ! -d "$DIR_FUZZ_IN" ]; then
    git clone --depth=1 https://github.com/umkoin-core/qa-assets "${DIR_QA_ASSETS}"
  fi
  (
    cd "${DIR_QA_ASSETS}"
...
if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
  bash -c "${TEST_RUNNER_ENV} DIR_UNIT_TEST_DATA=${DIR_UNIT_TEST_DATA} LD_LIBRARY_PATH=${DEPENDS_DIR}/${HOST}/lib ${BASE_OUTDIR}/bin/test_umkoin --catch_system_errors=no -l test_suite"
fi
...
if [ "${RUN_TIDY}" = "true" ]; then
  ...
  ( run-clang-tidy-"${TIDY_LLVM_V}" -quiet -load="/tidy-build/libumkoin-tidy.so" "${MAKEJOBS}" ) | grep -C5 "error"
