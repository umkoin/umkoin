# bash programmable completion for umkoin(1) wrapper
...
_umkoin() {
    ...
    local umkoin subcmd offset

    # save and use original argument to invoke umkoin for help
    ...
    case "$subcmd" in
        gui|node)
            _umkoin_wrap umkoind "$offset"
            return 0
            ;;
        rpc)
            _umkoin_wrap umkoin-cli "$offset"
            return 0
            ;;
        tx)
            _umkoin_wrap umkoin-tx "$offset"
            return 0
            ;;
    esac
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                options=$($umkoin help 2>&1 | awk '{ for(i=1;i<=NF;i++) if ($i~/^--/) { sub(/=.*/, "=",$i); print $i } }' )
            fi
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$($umkoin help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
