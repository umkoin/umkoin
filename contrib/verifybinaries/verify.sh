...
###   umkoincore.org and umkoin.org and compares them.
...
WORKINGDIR="/tmp/umkoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://umkoincore.org"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'umkoin-',
...
if [ $? -ne 0 ]; then
   echo "umkoin.org failed to provide signature file, but umkoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "umkoin.org and umkoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Umkoin Core binary release signing key installed?"
   fi
