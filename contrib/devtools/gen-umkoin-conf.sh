...
EXAMPLE_CONF_FILE=${EXAMPLE_CONF_FILE:-$SHARE_EXAMPLES_DIR/umkoin.conf}

[ ! -x "$UMKOIND" ] && echo "$UMKOIND not found or not executable." && exit 1

DIRTY=""
VERSION_OUTPUT=$($UMKOIND --version)
...
if [ -n "$DIRTY" ]
then
  echo -e "WARNING: $UMKOIND was built from a dirty tree.\n"
  echo -e "To safely generate a umkoin.conf file, please commit your changes to $UMKOIND, rebuild, then run this script again.\n"
fi

echo 'Generating example umkoin.conf file in share/examples/'
...
## Generated by contrib/devtools/gen-umkoin-conf.sh.
##
...
# parse the output from umkoind --help
