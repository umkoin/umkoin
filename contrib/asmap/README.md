...
be used in Umkoin Core with the `-asmap` option.
...
to binary before being used with Umkoin Core.
...
the `umkoin-cli getnodeaddresses` command.
...
```
umkoin-cli getnodeaddresses 0 > addrs.json
```
