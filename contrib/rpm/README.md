...
The RPM spec file provided here is for Umkoin-Core 0.12.0 and builds on CentOS
...
[Umkoin git repository](https://github.com/vmta/umkoin) but are not part of
...
Currently only `Source10` is used, for the example `umkoin.conf` file.
...
in the umkoin git repository. At present only an SVG version of the Umkoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the umkoin
...
needed when 0.12.1 is released, a proper fix is already in the Umkoin git
master branch.
...
idea to build Umkoin Core against a static version of that library compiled
...
The `umkoin.svg` file is from the source listed as `Source100`. It is used as
...
Umkoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
4. I renamed the `umkoin` package that contains the Qt GUI to `umkoin-core` as that appears to be how the general population refers to it, in contrast to `umkoin-xt` or `umkoin-classic`. I wanted to make sure the general population knows what they are getting when installing the GUI package.

As far as minor differences, I generally prefer to assign the file permissions in the `%files` portion of an RPM spec file rather than specifying the permissions of a file during `%install` and other minor things like that are largely just cosmetic.
