...
# Setup the directory where our Umkoin Core build for HOST will occur
...
make -C depends --jobs="$MAX_JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_i686_linux='-platform linux-g++ -xplatform umkoin-linux-g++'
...
if [ -z "$(find "${OUTDIR}/src" -name 'umkoin-*.tar.gz')" ]; then
    ./autogen.sh
    env CONFIG_SITE="${BASEPREFIX}/${HOST}/share/config.site" ./configure --prefix=/
    make dist GZIP_ENV='-9n' ${V:+V=1}
    mkdir -p "${OUTDIR}/src"
    mv "$(find "${PWD}" -name 'umkoin-*.tar.gz')" "${OUTDIR}/src/"
fi

# Determine the full path to our source tarball
SOURCEDIST="$(find "${OUTDIR}/src" -name 'umkoin-*.tar.gz')"
# Determine our distribution name (e.g. umkoin-0.18.0)
...
(
    ...
    # Setup the directory where our Umkoin Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Umkoin Core to $INSTALLPATH
...
case "$HOST" in
    *mingw*)
        ...
        (
            cd ./windeploy
            mkdir unsigned
            cp --target-directory=unsigned/ "$OUTDIR"/umkoin-*-setup-unsigned.exe
            find . -print0 \
                | sort --zero-terminated \
                | tar --create --no-recursion --mode='u+rw,go+r-w,a+X' --null --files-from=- \
