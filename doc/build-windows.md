...
and is the platform used to build the Umkoin Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Umkoin Core applications.
...
causes two of the umkoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the umkoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the umkoin source code).
