...
application bridge enabled. The following routers are recommended for use with Umkoin Core:
...
Once an I2P router with SAM enabled is up and running, use the following Umkoin
...
debug log about your I2P configuration and connections. Run `umkoin-cli help
...
I2P support was added to Umkoin Core in version 22.0 and there may be fewer I2P
...
The first time Umkoin Core connects to the I2P router, it automatically
...
`i2p_private_key` in the Umkoin Core data directory.  The persistent I2P
...
## Fetching I2P-related information from Umkoin Core

There are several ways to see your I2P address in Umkoin Core if accepting
...
You can use the `getnodeaddresses` RPC to fetch a number of I2P peers known to your node; run `umkoin-cli help getnodeaddresses` for details.
...
To allow future upgrades to newer versions of SAM, Umkoin Core sets its
listening port to 0 when listening for incoming I2P connections and advertises
its own I2P address with port 0. Furthermore, it will not attempt to connect to
I2P addresses with a non-zero port number because with SAM v3.1 the destination
port (`TO_PORT`) is always set to 0 and is not in the control of Umkoin Core.
...
It's important that the nodes of a popular application like Umkoin contribute
...
## Bundling I2P in a Umkoin application

Please see the "General Guidance for Developers" section in https://geti2p.net/en/docs/api/samv3
if you are developing a downstream application that may be bundling I2P with Umkoin.

## Privacy recommendations

- Operating a node that listens on multiple networks (e.g. IPv4 and I2P) can help
  strengthen the Umkoin network, as nodes in this configuration (i.e. bridge nodes) increase
