...
The first time Umkoin Core connects to the I2P router, if
...
file named `i2p_private_key` in the Umkoin Core data directory. The persistent
...
If an I2P node is not accepting incoming connections, then Umkoin Core uses
...
from programs which are connecting via the SAM proxy, e.g. Umkoin Core, you
