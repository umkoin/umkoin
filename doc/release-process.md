...
* Update umkoin.conf and commit changes if they exist, see [gen-umkoin-conf.sh](/contrib/devtools/README.md#gen-umkoin-confsh).
...
* Update the following variables in [`src/kernel/chainparams.cpp`](/src/kernel/chainparams.cpp) for mainnet, testnet, and signet:
  ...
  - `m_assumeutxo_data` array should be appended to with the values returned by calling `umkoin-cli -rpcclienttimeout=0 -named dumptxoutset utxo.dat rollback=<height or hash>`
...
- Translations on Transifex:
    ...
    - Create [a new resource](https://app.transifex.com/umkoin/umkoin-core/content/) named after the major version with the slug `qt-translation-<RRR>x`, where `RRR` is the major branch number padded with zeros. Use `src/qt/locale/umkoin_en.xlf` to create it.
...
In the `guix-build-${VERSION}/output/x86_64-apple-darwin` and `guix-build-${VERSION}/output/arm64-apple-darwin` directories:
    ...
    tar xf umkoin-${VERSION}-${ARCH}-apple-darwin-codesigning.tar.gz
...
In the `guix-build-${VERSION}/output/x86_64-w64-mingw32` directory:
    ...
    tar xf umkoin-${VERSION}-win64-codesigning.tar.gz
...
- Upload to the www.umkoin.org server:
    1. The contents of each `./umkoin/guix-build-${VERSION}/output/${HOST}/` directory.
