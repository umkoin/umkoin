# User-space, Statically Defined Tracing (USDT) for Umkoin Core

Umkoin Core includes statically defined tracepoints to allow for more
...
**Note**: Umkoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Umkoin Core
...
Multiple tools can list the available tracepoints in a `umkoind` binary with
USDT support.
...
```
$ gdb ./build/bin/umkoind
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/bin/umkoind
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/bin/umkoind
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/bin/umkoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Umkoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/bin/umkoind | grep NT_STAPSDT -A 4 -B 2
