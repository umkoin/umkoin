Umkoin Core version 0.12.0 is now available from:

  <https://umkoin.org/bin/umkoin-core-0.12.0/>
...
pre-0.12 versions of Umkoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Umkoin Core version 0.11 or
earlier.
...
ECDSA signatures inside Umkoin transactions now use validation using
...
(https://github.com/umkoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Umkoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Umkoin Core was to raise the default minimum
relay fee.

Umkoin Core 0.12 will have a strict maximum size on the mempool. The
...
Umkoin Core 0.12 also introduces new default policy limits on the length and
...
Umkoin Core 0.12 nodes. Umkoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/umkoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Umkoin Core 0.12 does not yet have support for
...
Umkoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Umkoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Umkoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Umkoin Core 0.13, and it is at this time undecided whether the
...
Umkoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Umkoin Core automatically creates a hidden service to listen on, without
manual configuration. Umkoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Umkoin Core is listening, and
...
Umkoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Umkoin
Core to use floating fees. By default, Umkoin Core will use floating
fees.
...
(default: `0.0002` UMK/kB).

At all times, Umkoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) UMK.
Furthermore, Umkoin Core will never create transactions paying less than
...
notes of v0.11.0](https://github.com/vmta/umkoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/umkoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/umkoin/bips/blob/master/bip-0065.mediawiki)
...
Then, to tunnel a SSL connection on 26332 to a RPC server bound on localhost on port 16332 do:

    stunnel -d 26332 -r 127.0.0.1:16332 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /umkoinrpc>
        ...
        # get the <hash> from the shell with: base64 <<< umkoinrpc:<password>
    </Location>
...
- #6269 `d4565b6` gitian: Use the new umkoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #7209 `f3ad812` test: don't override UMKOIND and UMKOINCLI if they're set (Wladimir J. van der Laan)
...
- #6401 `6db53b4` Add UMKOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
...
- #6504 `b6fee6b` Rationalize currency unit to "UMK" (Ross Nicoll)
