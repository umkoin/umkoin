Umkoin Core version 0.16.0 is now available from:

  <http://www.umkoin.org/bin/umkoin-core-0.16.0/>
...
Umkoin Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
umkoin data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Umkoin Core is now 4.8. No effort will be
...
- umkoin-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- umkoin-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.
...
- #11970 `b7450cd` Add test coverage for umkoin-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to umkoin-cli to allow RPC password to be read from standard input (jharvell)
...
- #11621 `1f7695b` [build] Add temp_umkoin_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` umkoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.umkoin doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for umkoin-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in umkoin-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop umkoind (OmeGak)
...
- #10529 `331352f` Improve umkoind systemd service file (Flowdalic)
