Umkoin Core version 0.17.0 is now available from:

  <http://www.umkoin.org/bin/umkoin-core-0.17.0/>
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the umkoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=umkoin
    test.uacomment=umkoin-testnet
...
be used in V0.17 by starting umkoind with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Umkoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Umkoin transactions that are not fully signed
...
Overall, the construction of a fully signed Umkoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Umkoin transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in UMK. This new field deprecates previous fee fields, such as
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart umkoind with
...
- When running umkoind with `-debug` but without `-daemon`, logging to stdout
...
- #11962 `0a01843` add seed.umkoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in UMK (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI umkoin:// (krab)
...
- #13528 `b877c39` Move UmkoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libumkoin-util to libumkoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for umkoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling umkoin-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use UmkoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to umkoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use UmkoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use UmkoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use UmkoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for UmkoinTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_umkoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.umkoin is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_umkoin` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in umkoind systemd service file (Flowdalic)
- #12652 `55f490a` umkoin-cli: Provide a better error message when umkoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to umkoind in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for umkoin address in translations (kaplanmaxe)
...
- #13603 `453ae5e` umkoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix umkoin-cli --version (Empact)
...
- #12877 `2b54155` Use umkoind in Tor documentation (knoxcard)
...
- #13346 `60f0358` update umkoin-dot-org links in release-process.md (fanquake)
...
- #13905 `73a09b4` fixed umkoin-cli -help output for help2man (hebasto)
