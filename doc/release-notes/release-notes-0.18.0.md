Umkoin Core version 0.18.0 is now available from:
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/umkoin/umkoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:6332:6332` (this is an extra `:6332` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](http://www.umkoin.org/en/contact.php) the Umkoin Core
...
The systemd init file (`contrib/init/umkoind.service`) has been changed
to use `/var/lib/umkoind` as the data directory instead of
`~umkoin/.umkoin`. This change makes Umkoin Core more consistent with
...
`datadir` option in `/etc/umkoin/umkoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/umkoin/umkoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/umkoin/umkoin/blob/master/doc/JSON-RPC-interface.md)
...
- A new document introduces Umkoin Core's BIP174 [Partially-Signed
  Umkoin Transactions
  (PSBT)](https://github.com/umkoin/umkoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/umkoin/umkoin/blob/master/doc/descriptors.md)
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/umkoin/umkoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/umkoin/umkoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Umkoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Umkoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/umkoin/umkoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/umkoin/umkoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
This section describes planned changes to Umkoin Core that may affect
other Umkoin software and services.

- Since version 0.16.0, Umkoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Umkoin Core 0.20 (expected about
  a year after 0.18), Umkoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Umkoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Umkoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Umkoin Core
...
- A sub-project of Umkoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Umkoin Core.  See their [project
  page](https://github.com/umkoin/HWI#readme) for details.
...
- On macOS, Umkoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `umkoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libumkoin_server.a` (jnewbery)
...
- #13926 [Tools] umkoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like umkoind (MarcoFalke)
...
- #14521 qt, docs: Fix `umkoin-qt -version` output formatting (hebasto)
...
- #11625 Add UmkoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libumkoin_server` (laanwj)
...
- #15154 Configure: umkoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/umkoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Umkoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libumkoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in umkoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Umkoin Core (Christewart)
...
- #12246 Bugfix: Only run umkoin-tx tests when umkoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_umkoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_umkoin` (MarcoFalke)
...
- #14186 umkoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_umkoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update umkoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/umkoin-conf.md (hebasto)
...
- #15354 Add missing `umkoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct umkoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support umkoin folder on OSX (JimmyMow)
