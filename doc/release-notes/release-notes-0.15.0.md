...
Bitcoin Core 0.15.0 contains the following changes to the RPC interface and `umkoin-cli` for multi-wallet:

* When running Bitcoin Core with a single wallet, there are **no** changes to the RPC interface or `umkoin-cli`. All RPC calls and `umkoin-cli` commands continue to work as before.
