...
Umkoin Core version 0.20.0 is now available from:

  <http://www.umkoin.org/bin/umkoin-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Umkoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Umkoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Umkoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Umkoin Core. Umkoin Core logs debug messages
...
- OpenSSL is no longer used by Umkoin Core.  (#17265)

- BIP70 support has been fully removed from Umkoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN. See [`umkoind help` for more information. This option is
  experimental and subject to removal or breaking changes in future
  releases, so the legacy /16 prefix mapping of IP addresses remains
  the default.  (#16702)

Updated settings
----------------

- All custom settings configured when Umkoin Core starts are now
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple Umkoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Umkoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitcoin/HWI) for
  signing.  Future versions of Umkoin Core should support a GUI option
...
- The `umkoin-cli` utility used with the `-getinfo` parameter now
...
- The `umkoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=6333` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `umkoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_umkoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBUMKOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_umkoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBUMKOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for umkoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate umkoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest umkoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_umkoin` build of `*.raw` (MarcoFalke)
...
- #16983 Add detailed info about Umkoin Core files (hebasto)
...
- #17119 Fix broken umkoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from umkoin-qt help message and manpage (laanwj)
...
- #17648 Rename wallet-tool references to umkoin-wallet (hel-o)
...
- #18341 Replace remaining literal UMK with `CURRENCY_UNIT` (domob1812)
...
- #16115 On umkoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from umkoin-cli -getinfo (malevolent)
