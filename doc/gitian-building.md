...
*Setup instructions for a Gitian build of Umkoin Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Umkoin
...
- [Building Umkoin Core](#building-umkoin-core)
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/umkoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/umkoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/umkoin-osx-build.assert
```
