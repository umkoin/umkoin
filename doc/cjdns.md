# CJDNS support in Umkoin Core

It is possible to run Umkoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Umkoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Umkoin network and individual nodes.
...
before it will work with your Umkoin Core node. This is described in steps
...
## Run Umkoin Core with CJDNS

Once you are connected to the CJDNS network, the following Umkoin Core
...
When enabled, this option tells Umkoin Core that it is running in an
...
IPv6 local network. This helps Umkoin Core perform better address management:
...
CJDNS support was added to Umkoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `umkoin-cli -addrinfo` to see the
...
## CJDNS-related information in Umkoin Core

There are several ways to see your CJDNS address in Umkoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `umkoin-cli -netinfo 4`
