# RapidCheck property-based testing for Umkoin Core

## Concept

Property-based testing is experimentally being added to Umkoin Core with
...
If RapidCheck is installed, Umkoin Core will automatically run the
...
3. Configure Umkoin Core with RapidCheck.

    `cd` to the directory of your local umkoin repository and run
...
4. Build Umkoin Core with RapidCheck.
    ...
    Making all in src
    [...]
    CXX      test/gen/test_umkoin-crypto_gen.o
    CXX      test/test_umkoin-key_properties.o
    ```
...
That's it! You are now running property-based tests in Umkoin Core.
