...
Umkoin Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while umkoind is running with the `logging`
RPC.  For instance, launching umkoind with `-debug` or `-debug=1` will turn on
...
Umkoin Core is a multi-threaded application, and deadlocks or other
...
You can then use the `-debug=lock` configuration option at umkoind startup or
`umkoin-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `umkoin-cli logging [] '["lock"]'`.
...
# A coverage report will now be accessible at `./build/test_umkoin.coverage/index.html`,
...
```shell
llvm-cov show \
    --object=build/bin/test_umkoin \
    --object=build/bin/umkoind \
    ...
    --project-title="Umkoin Core Coverage Report"
```
...
llvm-cov show \
    ...
    --show-expansions \
    --output-dir=build/coverage_report \
    --project-title="Umkoin Core Fuzz Coverage Report"
```
...
To profile a running umkoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep umkoind` -- sleep 60
```
...
Umkoin Core can be compiled with various "sanitizers" enabled, which add
...
pay attention to for reviewers of Umkoin Core code.
...
Normally, these are maintained by active developers of Umkoin Core, in which case
...
a Umkoin Core subtree, so that they can be integrated quickly. Cosmetic changes
...
The tool instructions also include a list of the subtrees managed by Umkoin Core.
...
- src/leveldb
  - Upstream at https://github.com/google/leveldb ; maintained by Google. Open
    important PRs to the Umkoin Core subtree to avoid delay.
...
file descriptors, it will cause problems with Umkoin's `select()` loop, because
...
compatibility between nodes. This happened in Umkoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Umkoin's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/umkoin/umkoin/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/umkoin/umkoin/commit/8922d7f6b751a3e6b3b9f6fb7961c442877fb65a)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/umkoin/umkoin/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `umkoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `umkoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Umkoin Core, the wallet was always
...
Umkoin Core uses a structured process for deprecating and removing features to give
...
- The RPC help text must mention the deprecation and the `-deprecatedrpc=<feature>` flag
  ...
  ```
  "\nDeprecated in v25.0, use the `newfoo` RPC instead. Start umkoind with"
...
- Interface method parameter and return types should either be serializable or
  ...
  // Bad: returns umksignals connection specific to local process
  using TipChangedFn = std::function<void(int block_height, int64_t block_time)>;
  virtual umksignals::scoped_connection connectTipChanged(TipChangedFn fn) = 0;
  ```
