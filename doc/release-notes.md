...
- The `enablebip61` option (introduced in Umkoin Core 0.17.0) is
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  run [...] -p 127.0.0.1:6332:6332` (this is an extra `:6332` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  (and may want to consider letting the Umkoin Core project know about their
...
- The `getaddressinfo` RPC now returns an additional `solvable` boolean
  field when Umkoin Core knows enough about the address's scriptPubKey,
...
- The `importprivkey` RPC will preserve previously-set labels for
  ...
  label "cold wallet" in earlier releases of Umkoin Core, subsequently
