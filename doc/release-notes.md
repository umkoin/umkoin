...
Umkoin Core version 0.21.0 is now available from:

  <http://www.umkoin.org/bin/umkoin-core-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  Version 2 hidden services are still fully supported by Umkoin Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Umkoin Core will no longer
...
- A new `umkoin-cli -netinfo` command provides a network peer connections
...
- The `connections` field of `umkoin-cli -getinfo` is now expanded to return a JSON
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `umkoin-wallet` tool which performs the salvage
...
Umkoin Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`umkoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Umkoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `umkoin-cli createwallet` or `umkoin-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/umkoin/umkoin/issues).
...
Users who already have a Umkoin Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Umkoin Core.
...
The change to using descriptors changes the default derivation paths used by Umkoin Core
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in UMK per 1,000
  vbytes (UMK/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from UMK/kB
  to sat/vB and the help documentation in UMK/kB is updated to UMK/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from UMK/kvB to sat/vB and the
  ...
  difference between UMK/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in UMK/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Umkoin Core is
...
- #19284 Add seed.umkoin.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in umkoin-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add umkoin-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in umkoin-cli (laanwj)
...
### Build system
- #18504 Drop umkoin-tx and umkoin-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in UMKOIN_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for umkoin-cli -rpcwait (jonatack)
...
- #20172 util: Do not use gargs global in argsmanager member functions (hebasto)
...
- #18739 Document how to fuzz Umkoin Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitreumkoin)
...
- #18817 Document differences in umkoind and umkoin-qt locale handling (practicalswift)
...
- #20170 Tiny followups to new getpeerinfo connection type field (amitiuttarwar)
