...
`./`               | `banlist.dat`         | Stores the addresses/subnets of banned nodes (deprecated). `umkoind` or `umkoin-qt` no longer save the banlist to this file, but read it on startup if `banlist.json` is not present.
...
These subdirectories and files are no longer used by Umkoin Core:
