...
To enable [descriptor wallets](https://github.com/umkoin/umkoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Umkoin Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.dmg` containing the Umkoin Core application using `make deploy`.
...
Umkoin Core should now be available at `./src/umkoind`.
If you compiled support for the GUI, it should be available at `./src/qt/umkoin-qt`.
...
./src/qt/umkoin-qt -server # Starts the umkoin-qt server mode, allows umkoin-cli control
```
