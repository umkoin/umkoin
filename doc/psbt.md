# PSBT Howto for Umkoin Core

Since Umkoin Core 0.17, an RPC interface exists for Partially Signed Umkoin
...
PSBT is an interchange format for Umkoin transactions that are not fully signed
...
#### Multisig with multiple Umkoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Umkoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `umkoin-cli` is used.
...
Later, when *V* UMK has been received on *Amulti*, and Bob and Carol want to
