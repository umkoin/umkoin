...
#ifndef UMKOIN_NODE_INTERFACE_UI_H
#define UMKOIN_NODE_INTERFACE_UI_H
...
public:
    ...
    /** Show message box. */
    umksignals::signal<void(const bilingual_str& message, unsigned int style)> ThreadSafeMessageBox;
    ...
    umksignals::signal<bool(const bilingual_str& message, const std::string& noninteractive_message, unsigned int style), umksignals::any_of> ThreadSafeQuestion;

    /** Progress message during initialization. */
    umksignals::signal<void(const std::string& message)> InitMessage;

    /** Wallet loader created. */
    umksignals::signal<void()> InitWallet;

    /** Number of network connections changed. */
    umksignals::signal<void(int newNumConnections)> NotifyNumConnectionsChanged;

    /** Network activity state changed. */
    umksignals::signal<void(bool networkActive)> NotifyNetworkActiveChanged;

    /**
     * Status bar alerts changed.
     */
    umksignals::signal<void()> NotifyAlertChanged;

    /**
     * Show progress e.g. for verifychain.
     * resume_possible indicates shutting down now will result in the current progress action resuming upon restart.
     */
    umksignals::signal<void(const std::string& title, int nProgress, bool resume_possible)> ShowProgress;

    /** New block has been accepted */
    umksignals::signal<void(SynchronizationState, const CBlockIndex& block, double verification_progress)> NotifyBlockTip;

    /** Best header has changed */
    umksignals::signal<void(SynchronizationState, int64_t height, int64_t timestamp, bool presync)> NotifyHeaderTip;

    /** Banlist did change. */
    umksignals::signal<void(void)> BannedListChanged;
};
...
#endif // UMKOIN_NODE_INTERFACE_UI_H
