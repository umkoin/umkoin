...
Options:
  -m, --multiprocess     Run multiprocess binaries umkoin-node, umkoin-gui.
  -M, --monolithic       Run monolithic binaries umkoind, umkoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show this help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'umkoin-qt [ARGS]' or 'umkoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'umkoind [ARGS]' or 'umkoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'umkoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'umkoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'umkoin-tx [ARGS]'.
...
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_umkoin [ARGS]'.
  chainstate [ARGS] Run umkoin kernel chainstate util, equivalent to running 'umkoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_umkoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_umkoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(cmd.use_multiprocess ? "umkoin-gui" : "umkoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(cmd.use_multiprocess ? "umkoin-node" : "umkoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("umkoin-cli");
            // Since "umkoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("umkoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("umkoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_umkoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("umkoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_umkoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_umkoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("umkoin-util");
        } else {
...
//! Execute the specified umkoind, umkoin-qt or other command line in `args`
...
//!             be a relative path to a umkoind, umkoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/umkoin ...
