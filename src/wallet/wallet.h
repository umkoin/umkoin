...
public:
     ...
     * umkoin-wallet tool code or by wallet migration code. If this is called
    ...
    umksignals::signal<void ()> NotifyUnload;

    /**
     * Address book entry changed.
     * @note called without lock cs_wallet held.
     */
    umksignals::signal<void(const CTxDestination& address,
     ...
     */
    umksignals::signal<void(const Txid& hashTx, ChangeType status)> NotifyTransactionChanged;

    /** Show progress e.g. for rescan */
    umksignals::signal<void (const std::string &title, int nProgress)> ShowProgress;

    /** Keypool has new keys */
    umksignals::signal<void ()> NotifyCanGetAddressesChanged;

    /**
     * Wallet status (encrypted, locked) changed.
     * Note: Called without locks held.
     */
    umksignals::signal<void (CWallet* wallet)> NotifyStatusChanged;
