...
#ifndef UMKOIN_IPC_PROCESS_H
#define UMKOIN_IPC_PROCESS_H
...
//! IPC process interface for spawning umkoin processes and serving requests
...
public:
    ...
    //! Canonize and connect to address, returning socket descriptor.
    virtual int connect(const fs::path& data_dir,
                        const std::string& dest_exe_name,
                        std::string& address) = 0;

    //! Create listening socket, bind and canonize address, and return socket descriptor.
