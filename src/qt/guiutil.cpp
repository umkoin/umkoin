...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Umkoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new UmkoinAddressEntryValidator(parent));
    widget->setCheckValidator(new UmkoinAddressCheckValidator(parent));
}

bool parseUmkoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no umkoin: URI
    if(!uri.isValid() || uri.scheme() != QString("umkoin"))
        return false;
...
bool openUmkoinConf()
{
    ...
    /* Open umkoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Umkoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Umkoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Umkoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Umkoin*.lnk
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a umkoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Umkoin\n";
        else
            optionFile << strprintf("Name=Umkoin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(CFArrayRef listSnapshot, LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the umkoin app
...
bool GetStartOnSystemStartup()
{
    CFURLRef umkoinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (umkoinAppUrl == nullptr) {
        return false;
    }

    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    CFArrayRef listSnapshot = LSSharedFileListCopySnapshot(loginItems, nullptr);
    bool res = (findStartupItemInList(listSnapshot, loginItems, umkoinAppUrl) != nullptr);
    CFRelease(umkoinAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    CFArrayRef listSnapshot = LSSharedFileListCopySnapshot(loginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(listSnapshot, loginItems, umkoinAppUrl);

    if(fAutoStart && !foundItem) {
        // add umkoin app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, umkoinAppUrl, nullptr, nullptr);
    }
