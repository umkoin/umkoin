...
/** Call node.updateRwSetting() with Umkoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Umkoin 22.x releases try to read these specific settings as
        ...
        // instead of strings, because umkoin 22.x will not try to read these.
...
//! Convert enabled/size values to umkoin -prune setting.
...
//! Get pruning enabled value to show in GUI from umkoin -prune setting.
...
//! Get pruning size value to show in GUI from umkoin -prune setting. If
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if umkoin.conf value
