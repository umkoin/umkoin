...
class UmkoinCore: public QObject
{
    Q_OBJECT
public:
    explicit UmkoinCore(interface::Node& node);
...
class UmkoinApplication: public QApplication
{
    Q_OBJECT
public:
    explicit UmkoinApplication(interface::Node& node, int &argc, char **argv);
    ~UmkoinApplication();
...
UmkoinCore::UmkoinCore(interface::Node& node) :
    QObject(), m_node(node)
{
}
...
UmkoinApplication::UmkoinApplication(interface::Node& node, int &argc, char **argv):
...
int main(int argc, char *argv[])
{
    ...
    try {
        node->readConfigFile(gArgs.GetArg("-conf", UMKOIN_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(0, QObject::tr(PACKAGE_NAME),
