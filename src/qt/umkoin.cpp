...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<UmkoinUnit>("UmkoinUnit");
#else
...
static bool InitSettings()
{
    ...
    if (!gArgs.ReadSettingsFile(&errors)) {
        std::string error = QT_TRANSLATE_NOOP("umkoin-core", "Settings file could not be read");
        std::string error_translated = QCoreApplication::translate("umkoin-core", error.c_str()).toStdString();
    ...
    if (!gArgs.WriteSettingsFile(&errors)) {
        std::string error = QT_TRANSLATE_NOOP("umkoin-core", "Settings file could not be written");
        std::string error_translated = QCoreApplication::translate("umkoin-core", error.c_str()).toStdString();
        InitError(Untranslated(strprintf("%s:\n%s\n", error, MakeUnorderedList(errors))));
...
void UmkoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    ...
    m_splash->show();
    connect(this, &UmkoinApplication::splashFinished, m_splash, &SplashScreen::finish);
    connect(this, &UmkoinApplication::requestedShutdown, m_splash, &QWidget::close);
}
