...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<UmkoinUnit>("UmkoinUnit");
}
...
bool UmkoinApplication::createOptionsModel(bool resetSettings)
{
...
void UmkoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool UmkoinApplication::baseInitialize()
{
...
void UmkoinApplication::InitPruneSetting(int64_t prune_MiB)
{
...
void UmkoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
...
void UmkoinApplication::handleNonFatalException(const QString& message)
{
...
bool UmkoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", UmkoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    umksignals::scoped_connection handler_message_box = ::uiInterface.ThreadSafeMessageBox_connect(noui_ThreadSafeMessageBox);
    umksignals::scoped_connection handler_question = ::uiInterface.ThreadSafeQuestion_connect(noui_ThreadSafeQuestion);
    umksignals::scoped_connection handler_init_message = ::uiInterface.InitMessage_connect(noui_InitMessage);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(UMKOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see umkoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see umkoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse umkoin.conf, determine network, switch to network specific
