...
//! Specify whether parameter should be parsed by umkoin-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make umkoin-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `umkoin-cli` attempt to parse the
 ...
 * `umkoin-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make umkoin-cli treat them as positional parameters when `-named` is used.
 * This prevents `umkoin-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so umkoin-cli
 ...
 *
 * For example, the command `umkoin-cli -named createwallet "my=wallet"`,
