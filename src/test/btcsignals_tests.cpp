...
{
    umksignals::signal<bool(), umksignals::any_of> sig0;
    ...
    {
        umksignals::scoped_connection conn0{sig0.connect(ReturnTrue)};
        umksignals::scoped_connection conn1{sig0.connect(ReturnFalse)};
        ret = sig0();
        BOOST_CHECK_EQUAL(ret, true);
        conn0.disconnect();
        ret = sig0();
...
BOOST_AUTO_TEST_CASE(thread_safety)
{
    ...
    std::thread extra_increment_injector([&conn0, &sig0, &val_non_det] {
        ...
        for (size_t i = 0; i < num_extra_conns; i++) {
            BOOST_CHECK(!sig0.empty());
            BOOST_CHECK(conn0.connected());
            umksignals::scoped_connection extra{sig0.connect([&val_non_det] { val_non_det++; })};
            if (i % 2 == 0) {
