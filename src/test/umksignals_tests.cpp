...
BOOST_AUTO_TEST_CASE(disconnects)
{
    ...
    {
        umksignals::scoped_connection scope(conn0);
    }
    BOOST_CHECK(!conn0.connected());
    BOOST_CHECK(sig0.empty());
    sig0(val);
...
{
    umksignals::signal<MoveOnlyData(int)> sig0;
...
{
    umksignals::signal<bool()> sig0;
    decltype(sig0)::result_type ret;
    ret = sig0();
    BOOST_CHECK(!ret);
    {
        umksignals::scoped_connection conn0 = sig0.connect(ReturnTrue);
        ret = sig0();
    ...
    BOOST_CHECK(!ret);
    {
        umksignals::scoped_connection conn1 = sig0.connect(ReturnTrue);
        umksignals::scoped_connection conn0 = sig0.connect(ReturnFalse);
        ret = sig0();
...
BOOST_AUTO_TEST_CASE(thread_safety)
{
    ...
    std::thread extra_increment_injector([&conn0, &sig0, &val] {
        ...
        std::vector<umksignals::scoped_connection> extra_conns;
...
{
    umksignals::connection conn0, conn1, conn2;
    umksignals::signal<void()> sig0;
