...
void ArgsManager::InterpretNegatedOption(std::string& key, std::string& val)
{
    if (key.substr(0, 3) == "-no") {
        ...
        // In an invocation like "umkoind -nofoo -foo" we want to unmark -foo
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No umkoin.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override umkoin.conf
