    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        std::string strUsage = strprintf("%s umkoin-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n";
            if (!gArgs.IsArgSet("-version")) {
                strUsage += "\n"
                    "umkoin-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n"
...
int main(int argc, char* argv[])
{
    ...
    if (method.empty()) {
        tfm::format(std::cerr, "No method provided. Run `umkoin-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
